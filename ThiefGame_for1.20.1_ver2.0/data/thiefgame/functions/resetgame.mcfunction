...
#なれっとの名前変更
data merge entity @e[type=glow_item_frame,tag=naletto,limit=1] {Item:{id:golden_carrot,Count:1,tag:{CustomModelData:1,display:{Name:'{"text":"「なれっと 教えて！」で僕に出来ることを教えるよ！"}'}}}}
#橋の修復
...
#各スキルのクールタイムをリセット
scoreboard players set @a invisible_time 60
...
tag @e[type=armor_stand,tag=rng,tag=show_answer] remove show_answer

#通知
tellraw @s "謎解きモードにリセットしました。"