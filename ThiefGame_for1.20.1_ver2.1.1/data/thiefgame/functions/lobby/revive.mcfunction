#蘇生
execute as @e[type=interaction,tag=soul_interaction] on target at @s if entity @s[team=thief,tag=!down] as @e[type=interaction,tag=soul_interaction,nbt={interaction:{}},sort=nearest] run scoreboard players add @s revive 1
...
#進捗トリガー
execute as @e[type=interaction,tag=soul_interaction,scores={revive=30..}] at @s run scoreboard players add @a[team=thief,tag=nurse] adv_troublesome 1
...
#看護師のパッシブ
execute if entity @a[tag=nurse] run function thiefgame:profession/passive/nurse