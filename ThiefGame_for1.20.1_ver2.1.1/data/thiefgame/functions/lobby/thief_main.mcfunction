...
##進捗トリガー
execute as @s[scores={Health=..20},tag=!live,tag=!in_lobby] at @s run function thiefgame:advancement/trigger/escape_profession/down
...
execute as @e[type=armor_stand,tag=bloodstain,scores={bloodstain_lifespan=..100}] at @s run particle item redstone_block ~ ~ ~ 0.2 0.001 0.2 0.00001 30 force @a
execute as @e[type=armor_stand,tag=bloodstain,scores={bloodstain_lifespan=140..}] run kill @s