#ステータスエフェクト
effect give @s[tag=!adversity,tag=!gambler] strength infinite 4 true
...
#通常の攻撃後無力化の解除
execute if entity @e[type=interaction,tag=ac5s] as @e[type=interaction,tag=ac5s] at @s run scoreboard players add @s attack_cooldown 1
execute if entity @e[type=interaction,tag=ac5s,scores={attack_cooldown=100..}] as @e[type=interaction,tag=ac5s,scores={attack_cooldown=100..}] at @s run kill @s