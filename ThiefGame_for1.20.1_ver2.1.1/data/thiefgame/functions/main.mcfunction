...
#鍵がかかっているドアを右クリックしたとき
execute as @e[type=interaction,tag=DI] on target unless predicate minecraft:open_door2 unless predicate minecraft:open_doortre run tellraw @s "鍵がかかっているようだ"
...
#血痕の寿命をカウント
execute if entity @e[type=armor_stand,tag=bloodstain] as @e[type=armor_stand,tag=bloodstain] run scoreboard players add @s bloodstain_lifespan 1
#役職
...
#全員が準備完了しているかチェック
execute as @e[type=armor_stand,tag=rng,tag=!start_count,tag=!timer] run function thiefgame:lobby/start/player_count/auto_start
#スタートカウント
...
#進捗トリガー関連
function thiefgame:advancement/trigger/adv_main

#OPホログラムUI
execute if entity @a[tag=in_lobby] as @a[tag=op,tag=in_lobby] at @s run function thiefgame:debug/hologram_ui/hologram_ui_main
